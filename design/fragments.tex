...
\caption{$[0,1]$ repetition\label{fig:srep}}
\end{figure}

Figure~\ref{fig:plus} shows how $A$ is converted to $A+$ or $A+?$. $k = ??$. Out edges are added from each $o_i$ to each $i_j$. In the greedy case, the out edges are prepended to each $o_i$'s edge list; in the nongreedy case, the out edges are appended.
...
\caption{$[1,\infty]$ repetition\label{fig:plus}}
\end{figure}

Figure~\ref{fig:star} shows how $A$ is converted to $A*$ or $A*?$. In addition to setting $k$, out edges are added from each $o_i$ to each $i_j$. In the greedy case, the out edges are prepended to each $o_i$'s edge list; in the nongreedy case, the out edges are appended.
