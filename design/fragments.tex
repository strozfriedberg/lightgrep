...
Figure~\ref{fig:plus} shows how $A$ is converted to $A+$ or $A+?$. $k = ??$. Out edges are added from each $o_i$ to each $i_j$ to create the necessary loops. Adding a plus does not affect the skippability of $A$, due to the fact that matching the empty string once is the same as matching the empty string any greater number of times; hence $A?.k = A??.k = A.k$.
