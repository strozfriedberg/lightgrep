...
Cli::Cli() : All(), Opts(new Options) {
  ...
  configOpts.add_options()
    ("file,f", po::value<std::vector<std::string>>(&Opts->KeyFiles)->composing()->value_name("FILE"),
      "Read one or more newline separated patterns from file")
    ("num_threads,j", po::value<unsigned int>(&Opts->NumThreads)->default_value(std::thread::hardware_concurrency())->value_name("THREADS"), "Number of worker threads to use");
...
std::shared_ptr<Options> Cli::parse(int argc, const char *const argv[]) const {
  ...
  po::store(po::command_line_parser(argc, argv).options(All).positional(PosOpts).run(), optsMap);
  po::notify(optsMap);
