...
void NFABuilder::repetition(const ParseNode& n) {
  if (n.Range.Min == 0) {
    ...
    else if (n.Range.Max == UNBOUNDED) {
      star(n);
      return;
    }
  }
  else if (n.Range.Min == 1 && n.Range.Max == UNBOUNDED) {
    plus(n);
    return;
  }
  ...
  while (!wind.empty()) {
    ...
    if (n->Left) {
      ...
      if ((n->Type == ParseNode::REPETITION ||
         ...
         !((n->Range.Min == 0 &&
           (n->Range.Max == 1 || n->Range.Max == UNBOUNDED)) ||
           (n->Range.Min == 1 && n->Range.Max == UNBOUNDED)))
      {
        ...
        if (n->Range.Min > 0) {
          ...
          for (uint32 i = 1; i < n->Range.Min; ++i) {
        ...
        if (n->Range.Min == n->Range.Max) {
          ...
          for (uint32 i = 1; i < n->Range.Max - n->Range.Min; ++i) {
            synth.push_back(std::shared_ptr<ParseNode>(
              new ParseNode(ParseNode::CONCATENATION, n->Left, none))
            );
            ParseNode* con = synth.back().get();
