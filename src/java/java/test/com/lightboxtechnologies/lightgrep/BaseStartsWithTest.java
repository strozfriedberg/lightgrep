...
public abstract class BaseStartsWithTest extends AbstractSearchTest {
  ...
  public static Collection<Object[]> data() throws UnsupportedEncodingException {
    return Arrays.asList(new Object[][] {
      ...
      { 0, 0, new Pat[]{ new Pat("a+b", new KeyOptions(), new String[]{ "ASCII" }) }, new ProgramOptions(), new ContextOptions(), "aaabaacabbabcacbaccbbbcbccca".getBytes("ASCII"), 0, "aaabaacabbabcacbaccbbbcbccca".getBytes("ASCII").length, 0, new SearchHit[]{ new SearchHit(0, 4, 0) }, null }
    });
  }

  public BaseStartsWithTest(int fsmSizeHint, int pmapSizeHint, Pat[] pats, ProgramOptions popts, ContextOptions copts, byte[] buf, int offset, int size, long startOffset, SearchHit[] ehits, Class<? extends Throwable> tclass) {
