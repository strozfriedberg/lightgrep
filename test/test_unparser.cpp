...
TEST_CASE("parseUnparse_a4_3_Test") {
  ParseTree tree;
  try {
    bisonParse({"a{4,3}", false, false}, tree);
  }
...
TEST_CASE("parseUnparse_CCd_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\d", false, false}, tree));
...
TEST_CASE("parseUnparse_CCs_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\s", false, false}, tree));
...
TEST_CASE("parseUnparse_CCw_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\w", false, false}, tree));
...
TEST_CASE("parseUnparse_LBaTozAToZCCdDot_PctBSHyphenPlusRB_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"[a-zA-Z\\d._%\\-+]", false, false}, tree));
  REQUIRE("[%+.0-9A-Z_a-z-]" == unparse(tree));
}
...
TEST_CASE("parseUnparse_BSx0_Test") {
  ParseTree tree;
  try {
    bisonParse({"\\x0", false, false}, tree);
  }
  catch (const std::runtime_error&) {
    // expected
    return;
...
TEST_CASE("parseUnparse_BSx00_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\x00", false, false}, tree));
  REQUIRE("\\x00" == unparse(tree));
}

TEST_CASE("parseUnparse_BS0_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\0", false, false}, tree));
  REQUIRE("\\x00" == unparse(tree));
}

TEST_CASE("parseUnparse_BS00_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\00", false, false}, tree));
  REQUIRE("\\x00" == unparse(tree));
}

TEST_CASE("parseUnparse_BS000_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\000", false, false}, tree));
  REQUIRE("\\x00" == unparse(tree));
}

TEST_CASE("parseUnparse_BS0000_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\0000", false, false}, tree));
  REQUIRE("\\x000" == unparse(tree));
}

TEST_CASE("parseUnparse_BS08_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\08", false, false}, tree));
  REQUIRE("\\x008" == unparse(tree));
}

TEST_CASE("parseUnparse_BS377_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"\\377", false, false}, tree));
...
TEST_CASE("parseUnparse_BS400_Test") {
  ParseTree tree;
  try {
    bisonParse({"\\400", false, false}, tree);
    REQUIRE(false);
  }
  catch (const std::runtime_error&) {
    // expected
...
TEST_CASE("parseUnparse_xBSq_Test") {
  ParseTree tree;
  try {
    bisonParse({"x\\q", false, false}, tree);
  }
  catch (const std::runtime_error&) {
    // expected
    return;
...
TEST_CASE("parseUnparse_invalid_utf8_Test") {
  ParseTree tree;
  try {
    bisonParse({"\xFF", false, false}, tree);
  }
  catch (const std::runtime_error&) {
    // expected
    return;
...
TEST_CASE("parseUnparse_many_levels_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"[[a]b[[c]d][[[e]]]]", false, false}, tree));
...
TEST_CASE("parseUnparse_a_and_a_and_a_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"[a&&a&&a]", false, false}, tree));
  REQUIRE("[a]" == unparse(tree));
}
...
TEST_CASE("parseUnparse_a_less_a_less_a_Test") {
  ParseTree tree;
  try {
    bisonParse({"[a--a--a]", false, false}, tree);
  }
  catch (const std::runtime_error&) {
    // expected
    return;
...
TEST_CASE("parseUnparse_a_to_z_less_a_to_z_Test") {
  ParseTree tree;
  try {
    bisonParse({"[[a-z]--[a-z]]", false, false}, tree);
  }
  catch (const std::runtime_error&) {
    // expected
    return;
...
TEST_CASE("parseUnparse_american_express_cc_number_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"3[47]\\d{2}[ \\-]?\\d{6}[ \\-]?\\d{5}", false, false}, tree));
...
TEST_CASE("parseUnparse_i_switch_09_Test") {
  ParseTree tree;
  REQUIRE(bisonParse({"(?i)((?-i)ab)cd", false, false}, tree));
  REQUIRE("ab[Cc][Dd]" == unparse(tree));
}
